...
**Phase 1-2 추가 패치 & Phase 2-1**: UX 보완 및 고정 거래 시스템 (2025-12-22) ✅
- [x] (1-3) 월별 내비게이션 월 이동 버그 (+2개월씩 건너뛰는 문제) ✅
- [x] (1-4) 거래 삭제 기능 정상화 (개별 삭제/선택 삭제) ✅
- [x] (1-5) 고정 거래 로직 구현 (월 이동 시 자동 생성/노출) ✅
- [x] (2-1) 거래 입력 모달 UX 개선 (할부 UI 정리, 고정 체크 위치 상향) ✅
- [x] (2-2) 모달 오버레이(백드롭) 잔존 버그 해결 ✅
- [x] (2-3) 자주 쓰는 사용처 퀵 선택 메뉴(스켈레톤) 구현 ✅
