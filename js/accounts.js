          ...
          <div class="account-header">
            <div>
              ...
              <div style="display: flex; gap: 8px; align-items: center; margin-top: 4px;">
                ...
                ${account.owner ? `<span style="font-size: 0.8rem; color: #6B7280; padding: 2px 8px; background: #F3F4F6; border-radius: 4px;">${account.owner}</span>` : ''}
              </div>
            </div>
...
function renderAccountTransactionTable(selectedAccount = 'all') {
  ...
  if (sortedData.length === 0) {
    tableHTML += `
      <tr>
        <td colspan="9" style="text-align:center; padding:40px;">입출금 내역이 없습니다</td>
      </tr>
    `;
  } else {
    ...
    sortedData.forEach(entry => {
      ...
      if (selectedAccountData) {
        ...
        // 전체 계좌 선택 시: 각 거래의 계좌별로 잔액 계산
        const entryAccount = entry.paymentDetail;
        if (entryAccount) {
      ...
      if (selectedAccountData && selectedAccountData.owner) {
        ownerText = selectedAccountData.owner;
      } else if (selectedAccount === 'all') {
        // 전체 계좌 선택 시 해당 거래의 계좌에서 소유자 찾기
        const accountForEntry = accountData.find(acc => acc.name === entry.paymentDetail);
        if (accountForEntry && accountForEntry.owner) {
      ...
      tableHTML += `
        <tr style="${rowStyle}">
          ...
          <td style="color: #6B7280; font-weight: 500;">${ownerText}</td>
