...
if (accountTransactionCardForm) {
  accountTransactionCardForm.addEventListener('submit', function(e) {
    ...
    const now = Date.now();
    // ========================================
    // 계좌 관리 카드 대금 등록용 거래 내역
    // ✅ Phase 1: transactionData가 아닌 accountCardPayments에 저장
    // 월별 현황(소비 리스트)에는 절대 표시되지 않음
    // ========================================
    const newEntry = {
      id: now,
      date: date,
    ...
    if (typeof saveAccountCardPayments === 'function') {
      ...
      console.error('saveAccountCardPayments 함수를 찾을 수 없습니다.');
    }
    ...
    // ✅ 월별 현황 탭은 갱신하지 않음 (카드 대금은 월별 소비 리스트에 표시되지 않음)
    // 월별 현황은 개별 지출 내역만 표시하고, 카드 대금 청구 총액은 표시하지 않음
    ...
    alert('카드 대금이 등록되었습니다!\n\n계좌 잔액에만 반영되며, 월별 현황 소비 리스트에는 표시되지 않습니다.');
    closeAccountTransactionModal();
  });
}
